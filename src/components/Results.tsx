...
export function ResultCard({ label, value, subtitle, highlight, icon }: ResultCardProps) {
  return (
    <div
      className={`rounded-lg border p-4 ${highlight ? 'border-2' : ''}`}
      style={{
...
export function ResultRow({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div
      className={`flex justify-between items-center py-2 ${bold ? 'border-t-2 font-bold' : 'border-t'}`}
  ...
  return (
    <details className="group border-b" style={{ borderColor: 'var(--border)' }}>
      ...
      >
        {question}
        <span
          className="ml-2 transition-transform group-open:rotate-45 text-xl font-light"
